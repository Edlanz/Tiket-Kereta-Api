...
const Gerbong = (myProp: props) => {
    return (
        <div className="w-full flex my-2 bg-slate-50 flex-wrap justify-between rounded-md shadow-md">
            <div className="p-3">
                ...
                <div className="w-full my-2">
                    {
                        myProp.item.seats.length == 0 ?
                            <div className="bg-sky-200 p-5 rounded-md">
                                Gerbong ini belom mempunyai kursi
                            </div> :
                            <div className="flex flex-wrap gap-3">
                                {
                                    myProp.item.seats.map((seat, index) => (<Seat key={`keySeat-${index}`}
                                    item={seat} />))
                                }
                            </div>
                    }
